...
\begin{lemma}[Generating algebras]
\label{CountablyGeneratedAlgebraIsSeparable}
\hypertarget{CountablyGeneratedAlgebraIsSeparable}
Take a finite or countable subset
\[ 
    \mathcal{F} = \left\{ f_1, f_2, \ldots \right\} \subseteq C_b(X). 
\]
Then, the subalgebra \( \left\langle \mathcal{F} \right\rangle  \) generated by \( \mathcal{F} \) contains functions of the form
\[ 
    \sum_{i=1}^n \prod_{j = 1}^{m_i} a_ij f_{ij}
\]
for some \( a_{ij} \in \mathbb{R} \) and \( f_{ij} \in \mathcal{F} \). We can get a countable dense set by taking all such expressions with rational coefficients.
...
\]
We need closedness for the left-to-right inclusion.
\end{proof}

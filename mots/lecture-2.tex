...
\hypertarget{CantorSetUniversalProperty}
 Every metrizable compact space \( K \) is a continuout image of \( \mathcal{C} \).
\end{thm}
...
A set \( A \subseteq \mathcal{C} \) is ???? by \( I \subseteq \mathbb{N} \), which we donte by \(A \sim I\) if for all \( x \in A \), \( y \in \mathcal{C} \) we have
\[ 
...
A set \( A \subseteq \mathcal{C} \) is clopen iff \( A \sim I \) for some finite \( I \subseteq \mathbb{N} \).
\end{lemma}
